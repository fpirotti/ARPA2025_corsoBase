...
v <- variogram(log(zinc) ~ 1, meuse, alpha = c(0, 30, 60, 90))
plot(v)
...
v.model.ini <- vgm("Sph")
v.fit <- fit.variogram(v, v.model.ini)
plot(v, v.fit)

v.model.ini <- vgm(1, "Exp", 1000, 0.1)
v.fit <- fit.variogram(v, v.model.ini)
plot(v, v.fit)
...
plot(v.dir, v.anis)
v.fit <- fit.variogram(v.dir, v.anis)

lz.sk <- krige(log(zinc) ~ 1, meuse, meuse.grid, v.fit)

dev.off()
plot(log(meuse$zinc))
abline(mean(log(meuse$zinc)))

spplot(lz.sk)
plot(lz.sk)

plot(terra::rast(lz.sk))
lz.sk
#####
...
lm.f <- lm(f, data=meuse)
lm.f
abline(lm.f, lwd=4, col="red")

vt <- variogram(f, meuse)
plot(vt)
vt.fit <- fit.variogram(vt, vgm(1, "Exp", 300, 1))
plot(vt, vt.fit)
lz.uk <- krige(f, meuse, meuse.grid,  vt.fit)

plot(terra::rast(lz.uk))
