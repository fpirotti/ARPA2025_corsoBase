...
Terra package per disegnare il risultato IDW
...
Esportiamo raster GeoTIFF per GIS

Conoscendo l'area, vicino a Maastricht - Paesi Bassi - applichiamo un SR
probabile e verifichiamo aprendo in GIS il raster salvato come GeoTIFF
dopo aver assegnato il[sistema di riferimento
EPSG:28992](https://epsg.io/28992)
...
## Stima il valore in una posizione
...
diversi lag ovvero a diverse distanze reciproche tra punti.
...
-   Serve a mantenere la relazione con la **varianza**:\
    quando $h→\infty$, la semivarianza tende alla **varianza della
...
Boulder](images/clipboard-1990818548.png){width="625"}
...
Tipico di concentrazioni di minerali
...
Tipico di valori di precipitazione
...
Fenomeni che oscillano nell'area
...
E' un grafico che mostra l'autocorrelazione dei valori dei dati a
diversi lag (spaziali o temporali).
...
Il variogramma può aiutare a decidere se la variabile dipendente è
stazionaria o meno.
...
all'interno dell'area di studio.Test di stazionarietà - variogramma (3)
...
-   Studio variografico di una variabile georeferenziata
    ...
    -   verifica stazionarietà e isotropia
...
Condizione di isotropia - verifica.

Esercizio - stima con una variabile completamente casuale

```{r}

#meuse$rand <- runif(nrow(meuse))
...
Scala e densità di campioni (vedi app online per simulare campionamenti)

La distanza di **cutoff**, che è la distanza massima fino alla quale le
coppie di punti vengono considerate, e il **width** è l’ampiezza
dell’intervallo di distanza all’interno del quale le coppie di punti
vengono mediate nei bin

```{r}
plot(variogram(log(zinc) ~ 1, meuse, cutoff = 2000, width = 50))
```
...
```{r}
v<-variogram(log(zinc) ~ 1, meuse, boundaries = c(0, 50,  100, seq(250, 1500, 250)))
plot(v)
```

## Stimare il modello ed i parametri

Gstat utilizza il metodo dei minimi quadrati per stimare i parametri di
un modello che riteniamo possa essere adatto. Se i dati non sono
coerenti con il modello, potrebbe dare una non-soluzione (singular fit)
e non trovare i parametri del modello.

Esercizio - fornite voi a mano un sill, nugget e range (vedi
documentazione vgm e [Variogramma](#variogramma))

```{r}
 # show.vgms()
# NB come mai fornisce un singular fit?
...
Se c'è anisotropia (l'autocorrelazione dipende dalla direzione oltre che
dalla distanza reciproca) possiamo integrarlo nel modello imponendola
...
Una volta trovato il modello adatto possiamo usarlo per l'interpolatore
usando la componente stocastica - ad esempio usiamo il modello che tiene
in considerazione la leggera anisotropia dei nostri dati.
...
## Interpolazione kriging - esercizio

Provate ad applicare la stessa cosa ad una variabile differente ed ad
utilizzare TERRA per creare i due plot.

```{r}
v.dir2 <- variogram(log(copper) ~ 1, meuse, alpha = (0:3) * 45)
v.anis <- vgm(0.6, "Sph", 1600, 0.05, anis = c(45, 0.3))
v.fit <- fit.variogram(v.dir2, v.anis)
plot(v.dir2,  v.fit)
lz.sk.cu <- krige(log(copper) ~ 1, meuse, meuse.grid, v.fit, beta = 5.9)
plot(terra::rast(lz.sk.cu), main="Rame" )
plot(terra::rast(lz.sk), main="Zinco" )
...
![](images/clipboard-1448665520.png){width="654"}

# TECNICHE DI KRIGING - Diversi Kriging {.Section1 .titlepage}

## Simple Kriging (SK)

Presuppone che la media della variabile regionalizzata sia nota.
...
sk <- krige(log(zinc)~1, meuse, meuse.grid, model=fit,
            beta=5.5)  # conosciamo la media e la forniamo 
```

## Ordinary Kriging (OK)

Assume che la media della variabile regionalizzata sia sconosciuta e
costante. Ampiamente utilizzato per la maggior parte delle applicazioni
**dove non c'è trend**. Fornisce stime e i relativi errori Si applica
nel caso di funzioni aleatorie stazionarie, ossia nel caso in cui la
media dei residui sia costante in tutto il dominio di studio
(stazionarietà!)
...
sk <- krige(log(zinc)~1, meuse, meuse.grid, model=fit )  # niente media, viene stimata con metodo dei minimi quadrati
```
...
Se la componente residua $\varepsilon(s)$ è una funzione stazionaria e
non è correlata al trend allora è possibile applicare la procedura di
Kriging al residuo e quindi effettuare Universal Kriging (UK).
...
Riassumendo; provate a cambiare alcuni parametri e visualizzare il
risultato
...
```{r  eval=FALSE}
edk <- krige(log(zinc)~dist, meuse, meuse.grid, model=fit)
```
...
```{r  eval=FALSE}
lm_fit <- lm(log(zinc)~dist, data=meuse@data)
meuse$resid <- residuals(lm_fit)

vg_res <- variogram(resid~1, meuse)
fit_res <- fit.variogram(vg_res, vgm("Sph"))

rk <- krige(resid~1, meuse, meuse.grid, model=fit_res)
rk$pred <- rk$var1.pred + predict(lm_fit, meuse.grid@data)
```
...
## Differenze chiave RK rispetto ad altri metodi:
...
Si ottiene trasformando una variabile continua in un indicatore (0/1),
oppure riducendo una variabile categorica a variabile binaria. Un
esempio è l’indicatore che vale 1 se lo zinco è **inferiore a 500 ppm**
e 0 altrimenti:

```{r eval=FALSE }

meuse$ind <- ifelse(meuse$zinc > 1000, 1, 0)

vg_ind <- variogram(ind~1, meuse)
fit_ind <- fit.variogram(vg_ind, vgm("Sph"))

ik <- krige(ind~1, meuse, meuse.grid, model=fit_ind)

## oppure (da bivand et al. )
# Definizione della formula indicatore
ind.f <- I(zinc < 500) ~ 1
# Stima del variogramma
ind.fit <- fit.variogram(variogram(ind.f, meuse), vgm(1, "Sph", 800, 1))
# Kriging per indicatori
ind.kr <- krige(ind.f, meuse, meuse.grid, model = ind.fit)
```
...
Esegui un'interpolazione con IDW (già fatta nel giorno 1 vedi [Esercizio
finale giorno 1]) ed una usando il kriging UK usando sqrt(dist) come
covariata - consegnare il raster usando TERRA per salvare come raster
GeoTIFF, includendo il sistema di riferimento corretto (vedi [Esportiamo
raster GeoTIFF per GIS] ), come [Esercizio finale giorno 1]
